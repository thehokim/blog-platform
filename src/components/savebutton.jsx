    ...
    const fetchSaveStatus = async () => {
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/posts/${postId}/save-status?user_id=${userId}`,
          {
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Ошибка получения статуса сохранения: ${response.status}`
          );
        }
  ...
  const handleSaveClick = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Ошибка обновления статуса сохранения: ${response.status} - ${errorText}`
        );
      }
